...
response = client.publish(
    ...
    # TargetArn='string',
    # PhoneNumber='string',
    Message='Hello from Hexa drone script',
    Subject='Boom',
...
print "\n\n COMPLETED SNS Publish to send text message  \n\n"
...
def listener(self, name, message):
    ...
    if "AD" in name: 
        print '\n\n\nReceived Non-ADS-B message containing an "AD" named: %s\n\n\n' % name

    # if "A" in name: 
    #     print '\n\n\nReceived Non-ADS-B message containing an "A" named: %s\n\n\n' % name
...
###################################
#   count to wait for ADSB message
###################################
# timer = 1000000000
# count = 0
# while count < timer:
...
###################################
#   wait 5 minutes for ADSB message
###################################
...
###################################
#   wait 5 minutes (for an ADSB message to arrive)
###################################
timeout = time.time() + 60*20   # 5 minutes from now
while True:
    test = 0
    if test == 5 or time.time() > timeout:
        break
